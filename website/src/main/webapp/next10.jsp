...
<head>
    ...
    <script>
        function redirectToPreviousPage() {
            window.location.href = 'day10.jsp';
        }
    </script>
    <style>
        body {
    ...
    <%
        ...
        try {
            ...
            if ("option1".equals(selectedOption)) {
                ...
                // 골드 부족 시 경고 처리
                if (gold >= 5) {
                    String updateQuery = "UPDATE users SET gold = gold - 5, soldiers = soldiers + 10 WHERE name = ?";
                ...
                // 옵션 2: Decrease the budget / Gold +5, soldiers -10
                // 군대 부족 시 경고 처리
                if (soldiers >= 10) {
                    String updateQuery = "UPDATE users SET gold = gold + 5, soldiers = soldiers - 10 WHERE name = ?";
                    pstmt = conn.prepareStatement(updateQuery);
