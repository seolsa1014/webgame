    ...
    <%
        ...
        try {
            ...
            if ("option1".equals(selectedOption)) {
                // 옵션 1: Confiscate cathedral assets / Gold + 10, happiness - 1
                // happiness 부족 시 die.jsp로 리디렉션
                if (happiness >= 1) {
                    // 옵션에 따라 업데이트
                    String updateQuery = "UPDATE users SET gold = gold + 10, happiness = happiness - 1 WHERE name = ?";
                    pstmt = conn.prepareStatement(updateQuery);
                    pstmt.setString(1, loggedInName);
                    pstmt.executeUpdate();
                } else {
                    // happiness 부족 시 die.jsp로 리디렉션
                    response.sendRedirect("die.jsp");
                    return;
                }
            } else if ("option2".equals(selectedOption)) {
                // 옵션 2: Leave it as it is (아무런 업데이트 없음)
            }
