...
<head>
    ...
    <script>
        function redirectToPreviousPage() {
            window.location.href = 'day14.jsp';
        }
    </script>
    <style>
        body {
    ...
    <%
        ...
        try {
            ...
            if ("option1".equals(selectedOption)) {
                ...
                if (gold >= 5) {
                	int check=0;
                	int randomChance = new Random().nextInt(2); // 0 또는 1
                    if (randomChance == 0) {
                       check = 1;
                    ...
                    if(check == 1)
                    {
                    	updateQuery = "UPDATE users SET gold = gold - 5, happiness = happiness + 1 WHERE name = ?";
                    }
                    else
                    {
                    	updateQuery = "UPDATE users SET gold = gold- 5, happiness = happiness - 1 WHERE name = ?";
                    ...
                    out.println("<script>window.location.href='day14.jsp';</script>");
                    return;
                }
            } 
            else if ("option2".equals(selectedOption)) {
                // 옵션 3: Cancel the restructuring (아무런 업데이트 없음)
            }
