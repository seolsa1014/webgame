...
<head>
    ...
    <style>
        body {
            ...
            height: 800px;
            background: url('css/day8.png') center/100% 100% no-repeat;
        }
    </style>
</head>
<body>
    <%
        ...
        try {
            ...
            if ("option1".equals(selectedOption)) {
                // 옵션 1: Retaliate and seize the loot / soldiers - 5, gold + 10
                // 부족한 자원이 있는 경우 경고 처리
                if (soldiers >= 5 && gold >= 10) {
                    // 옵션에 따라 업데이트
                    String updateQuery = "UPDATE users SET soldiers = soldiers - 5, gold = gold + 10 WHERE name = ?";
                    pstmt = conn.prepareStatement(updateQuery);
                    pstmt.setString(1, loggedInName);
                    pstmt.executeUpdate();
                } else {
                    // 부족한 자원이 있는 경우 경고창을 띄우고 이전 페이지로 이동
                    out.println("<script>alert('Not enough soldiers or gold!');</script>");
                    out.println("<script>window.location.href='day8.jsp';</script>");
                    return;
                }
            } else if ("option2".equals(selectedOption)) {
                // 옵션 2: Pay a ransom / Food - 5
                // 푸드 부족 시 경고 처리
                if (food >= 5) {
                    // 옵션에 따라 업데이트
                ...
                // 옵션 3: Do nothing / happiness - 1
                // happiness가 부족한 경우 die.html로 리디렉션
                if (happiness <= 1) {
