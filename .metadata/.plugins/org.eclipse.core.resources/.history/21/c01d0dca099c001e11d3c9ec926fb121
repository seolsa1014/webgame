...
<head>
    ...
    <script>
        function redirectToNextPage(option) {
            window.location.href = 'next9.jsp?option=' + option;
        }
    </script>
      <style>
    body {
    ...
    height: 800px;
    background: url('css/day9.png') center/100% 100% no-repeat;
}
    </style>
</head>
...
<div class="announcement">
    <p>Rumors have surfaced about issues with the donations coming from the cathedral in the kingdom. The issue of donations must be addressed.</p>
</div>

<div class="buttons">
    <button class="button" onclick="redirectToNextPage('option1')">Confiscate cathedral assets /  Gold + 10,happiness - 1</button>
    <button class="button" onclick="redirectToNextPage('option2')">Leave it as it is.</button>
</div>
